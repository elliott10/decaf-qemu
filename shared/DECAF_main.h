...
static inline void DECAF_flushTranslationBlock(uint32_t addr)
{
  ...
  CPU_FOREACH(env)
  {
    DECAF_flushTranslationBlock_env((CPUArchState*)(env->env_ptr), addr);
  }
}
...
static inline void DECAF_flushTranslationPage(uint32_t addr)
{
  ...
  CPU_FOREACH(env)
  {
    DECAF_flushTranslationPage_env((CPUArchState*)(env->env_ptr), addr);
  }
}
...
static inline void DECAF_flushTranslationCache(void)
{
  ...
  CPU_FOREACH(env)
  {
    tb_flush((CPUArchState*)(env->env_ptr));
  }
}
